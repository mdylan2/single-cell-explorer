...
build: clean build-client
	git ls-files server/ | grep -v 'backend/server/test/' | cpio -pdm $(BUILDDIR)
...
.PHONY: lint-server
lint-server: fmt-py
	flake8 server --per-file-ignores='tests/fixtures/dataset_config_outline.py:F821 tests/fixtures/server_config_outline.py:F821 tests/performance/scale_test_annotations.py:E501'
