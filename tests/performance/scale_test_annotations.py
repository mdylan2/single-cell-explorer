...
locust -f server/tests/test_server/performance/scale_test_annotations.py --headless -u 30 -r 10 --host https://api.cellxgene.dev.single-cell.czi.technology/cellxgene/e/ --run-time 5m 2>&1 | tee locust_dev_stats.txt

Or if you want to use the locust gui run:
locust -f server/tests/test_server/performance/scale_test_annotations.py -u 30 -r 10 --host https://api.cellxgene.dev.single-cell.czi.technology/cellxgene/e/

If you want to tests staging you'll need to substitute staging for dev in the host url
To tests prod you'll need to replace dev.single-cell.czi.technology with cziscience.com
If you'd like to tests additional datasets you'll need to add them to the dataset_urls array
