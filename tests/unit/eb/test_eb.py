...
class Elastic_Beanstalk_Test(unittest.TestCase):
    def test_run(self):
        tempdir = tempfile.TemporaryDirectory(dir=f"{PROJECT_ROOT}/server/server")
        ...
        subprocess.check_call(f"git ls-files . | cpio -pdm {tempdirname}", cwd=f"{PROJECT_ROOT}/server/server/eb",
                              shell=True)
    ...
    def test_config(self):
        check_config_script = os.path.join(PROJECT_ROOT, "server", "server", "eb", "check_config.py")
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            # tests failure mode (flask_secret_key not set)
            env = os.environ.copy()
