...
from server.test.test_server.unit.common.config import ConfigTests
...
class TestServerConfig(ConfigTests):
    ...
    def test_get_api_base_url_works(self):
        # tests the api_base_url feature, and that it can contain a path
        config = AppConfig()
        ...
        # tests the health check at the correct url
    ...
    def test_test_auth_only_in_insecure(self):
        ...
        config = self.get_config(auth_type="tests")
        with self.assertRaises(ConfigurationError):
            config.complete_config()
