...
from server import AnnotationsHostedTileDB
from server import AnnotationsLocalFile
from server.common.config.app_config import AppConfig
from server import BaseConfig
from tests import PROJECT_ROOT, FIXTURES_ROOT

from server import ConfigurationError
from tests.unit.common import ConfigTests
...
class TestDatasetConfig(ConfigTests):
    ...
    def test_multi_dataset(self):
        config = AppConfig()
        # tests for illegal url_dataroots
        for illegal in ("../b", "!$*", "\\n", "", "(bad)"):
        ...
        # tests that multi dataroots work end to end
        config.update_server_config(
            app__flask_secret_key="secret",
        ...
        # Change this default to tests if the dataroot overrides below work.
    ...
    def test_configfile_with_specialization(self):
        # tests that per_dataset_config config load the default config, then the specialized config
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            test_config = app_config.dataroot_config["tests"]

            # tests config from default
            ...
            # tests config from specialization
