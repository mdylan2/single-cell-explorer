...
from server import convert_color_to_hex_format, convert_anndata_category_colors_to_cxg_category_colors
...
class ColorsTest(unittest.TestCase):
    ...
    def test_anndata_colors_to_cxg_colors(self):
        ...
        # tests that invalid color formats raise an exception
        adata.uns["louvain_colors"][0] = "#NOTCOOL"
        with self.assertRaises(ColorFormatException):
            convert_anndata_category_colors_to_cxg_category_colors(adata)
        # tests that colors without a matching obs category are skipped
        adata = self._get_h5ad()
