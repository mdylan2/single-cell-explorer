...
class TestApplySchema(unittest.TestCase):
    ...
    def test_apply_schema(self, mock_get_ontology_label):
        mock_get_ontology_label.return_value = "tests label"
        ...
        self.assertListEqual(["tests label"], new_adata.obs["cell_type"].unique().tolist())
...
class TestFieldParsing(unittest.TestCase):
    ...
    def test_get_curie_and_label(self, mock_get_ontology_label):
        ...
        self.assertEqual(
            ...
            ("UBERON:1234", "tests label")
        )
        self.assertEqual(
            remix.get_curie_and_label("UBERON:1234 (cell culture)"),
            ("UBERON:1234 (cell culture)", "tests label (cell culture)")
        )
        self.assertEqual(
...
class TestFixupGeneSymbols(unittest.TestCase):
    ...
    def setUp(self):
        self.seurat_path = f"{PROJECT_ROOT}/server/tests/fixtures/schema_test_data/seurat_tutorial.h5ad"
        self.seurat_merged_path = f"{PROJECT_ROOT}/server/tests/fixtures/schema_test_data/seurat_tutorial_merged.h5ad"
        self.sctransform_path = f"{PROJECT_ROOT}/server/tests/fixtures/schema_test_data/sctransform.h5ad"
        self.sctransform_merged_path = f"{PROJECT_ROOT}/server/tests/fixtures/schema_test_data/sctransform_merged.h5ad"
    ...
    def test_fixup_gene_symbols_seurat(self):
        ...
        if not os.path.isfile(self.seurat_path):
            return unittest.skip(
                "Skipping gene symbol conversion tests because tests h5ads are not present. To create them, "
                "run server/tests/fixtures/schema_test_data/generate_test_data.sh"
            )
