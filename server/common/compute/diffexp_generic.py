...
from server import XApproximateDistribution
...
def diffexp_ttest(adaptor, maskA, maskB, top_n=8, diffexp_lfc_cutoff=0.01):
    ...
    Algorithm:
    - compute fold change
    - compute Welch's t-tests statistic and pvalue (w/ Bonferroni correction)
    ...
    Notes on alogrithm:
    - Welch's ttest provides basic statistics tests.
...
def diffexp_ttest_from_mean_var(meanA, varA, nA, meanB, varB, nB, top_n, diffexp_lfc_cutoff):
    ...
    # degrees of freedom for Welch's t-tests
    with np.errstate(divide="ignore", invalid="ignore"):
        dof = sum_vn ** 2 / (vnA ** 2 / (nA - 1) + vnB ** 2 / (nB - 1))
    dof[np.isnan(dof)] = 1

    # Welch's t-tests score calculation
    with np.errstate(divide="ignore", invalid="ignore"):
